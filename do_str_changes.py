from simulator import strategies_exploration
import sys
...
def main():
    ...
    str_changes = strategies_exploration(algorithm, game(1, citizens, f=multiplier),
                                        policy, 20, iterations=rounds, show=True)
    file_name = "str_changes_" + sys.argv[1] + "_" + sys.argv[2] + "_" + sys.argv[3] + "_" + sys.argv[4] + \
                "_" + sys.argv[5] + "_" + sys.argv[6] + "_" + sys.argv[7] + ".txt"
    print("Average Change: {0} | Deviation: {1}".format(np.mean(np.array(str_changes)), np.std(np.array(str_changes))))
    db = pd.DataFrame(data=np.array(str_changes))
    db.to_csv(path_or_buf="oldraw/" + file_name)
