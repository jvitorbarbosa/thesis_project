...
def main():
    data = pd.read_csv("raw/wealth_history_learning.csv")
    ...
    for agent in agents:
        reduced_data = reduced_data.append(aux_data.loc[aux_data['agent'] == agent]
                                           .loc[aux_data['round'] % 100 == 0])

    fig = px.bar(reduced_data, y='agent', x="wealth", animation_frame="round",
                  animation_group="agent", range_x=[0, 80000], orientation='h')
